...
class File {
    ...
    private static $sections =
        ...
        'profile_img' => ['path' => 'img/profile', 'secure_path' => false, 'type' => 'img', 'valid' => '/[\.\/](jpe?g|png)$/i', 'maxwidth' => 600, 'square' => true],
    ...
    public function upload($files) {
        ...
        foreach ($files as $file) {
            ...
            //setting file path
            $path = (@$options->secure_path) ? [storage_path(), self::$default_path, $options->path] : [public_path(), $options->path];
     ...
     */
    public static function deleteFile($file, $secure_path) {
        $path = explode('/', $file);

        //Este IF sirve realmente es cuando se implementa un sistema de carpetas por usuario
        //if (isset($path[2]) && \Auth::id() == $path[2]) {
        if (file_exists($file)) {
